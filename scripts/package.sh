...
rm -f "$CHROME_ZIP"
(cd "$CHROME_DIR" && zip -r "$CHROME_ZIP" . -x "*.DS_Store" "*_metadata/*" "*.git*" "*.github*" "tests/*" "test-sandbox.*" "test.*" "test-*" "*.md" "*.log" "*.tmp" "*.bak" "scratch/*" "debug*" "verify*")
...
rm -f "$FIREFOX_ZIP"
(cd "$FIREFOX_BUILD_DIR" && zip -r "$FIREFOX_ZIP" . -x "*.DS_Store" "*_metadata/*" "*.git*" "*.github*" "tests/*" "test-sandbox.*" "test.*" "test-*" "*.md" "*.log" "*.tmp" "*.bak" "scratch/*" "debug*" "verify*")
