...
export async function run() {
  ...
  runner.test('v1.0.5 Updates & Screenshots: v1.0.3 버전 표기 및 사이드패널 스크린샷 실존성 검증', () => {
    const htmlPath = path.join(DOCS_DIR, 'index.html');
    const htmlContent = fs.readFileSync(htmlPath, 'utf8');
    assert(htmlContent.includes('v1.0.5'), 'docs/index.html에 v1.0.5 버전 표기가 포함되어야 합니다.');
    assert(htmlContent.includes('v1.0.5 업데이트 내역'), 'docs/index.html에 v1.0.3 업데이트 내역 제목이 포함되어야 합니다.');
